...
export function reconcileGoliathState(state: HubState): HubState {
  ...
  next.nodes[CANONICAL_LINUX_NODE_ID] = {
    ...
    status: canonicalNode?.status === "reachable" || canonicalNode?.status === "active" ? canonicalNode.status : legacyNode?.status ?? "reachable",
    ...
    agentSurfaces: Array.from(new Set([...(canonicalNode?.agentSurfaces ?? []), ...(legacyNode?.agentSurfaces ?? []), "tailscale-ssh"])),
    capabilities: Array.from(new Set([...(canonicalNode?.capabilities ?? []), ...(legacyNode?.capabilities ?? []), "ssh", "council", "logs"])),
    reachability: {
      tailscale: true,
      ssh: true,
      ...
      notes: Array.from(new Set([...(canonicalNode?.reachability.notes ?? []), ...(legacyNode?.reachability.notes ?? []), "Tailscale-verified Linux server identity"]))
    },
    tags: Array.from(new Set([...(canonicalNode?.tags ?? []), ...(legacyNode?.tags ?? []), "tailscale", "server", "goliath"])),
    notes: mergedNotes,
  ...
  return hubStateSchema.parse(next);
}

export function applyGoliathIdentityReconciliation(statePath: string = resolveDataPath("hub-state.json")): HubState {
  ...
  const reconciled = reconcileGoliathState(current);
  fs.writeFileSync(statePath, JSON.stringify(reconciled, null, 2), "utf8");
  return reconciled;
}
